...
namespace JSON {
    ...
    inline void PrettyPrinter::printIndent(std::ostringstream &out) {
        for (unsigned int i = 0; i < currentIndent; ++i) {
            out << " ";
        }
    }

    inline void PrettyPrinter::printArray(const Value& value, std::ostringstream &out) {
        bool firstItem = true;
        out << "[";
    ...
    inline void PrettyPrinter::printObject(const Value& value, std::ostringstream &out) {
