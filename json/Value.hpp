...
namespace JSON {
    ...
    // A JSON::Value can represent every possible JSON type.
    struct Value {
    ...
    template <> inline int Value::as() const { return (int) as<double>(); }
    
    template <> inline unsigned int Value::as() const {
        return (unsigned int) std::abs(as<double>());
    }
    
    template <> inline long Value::as() const { return (long) as<double>(); }
    ...
    template <> inline std::string Value::as() const {
        ...
        case JSON_NULL:
            ...
            return "null";
        default:
            throw(std::runtime_error("Invalid convertion to string"));
        }
    }
    ...
    template <> inline bool Value::as() const {
        ...
        default:
            throw(std::runtime_error("Invalid conversion to boolean"));
        }
    }   
    ...
    template <> inline Array Value::as() const {
        ...
        case JSON_ARRAY:
            ...
            return std::get<JSON_ARRAY>(value);
        default:
            throw("Invalid conversion to array");
        }
    }
